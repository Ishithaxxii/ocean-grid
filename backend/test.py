...
def _grid_training_rows(grid_id: str) -> list[dict]:
    """
    Collect all profile observations belonging to one 5x5 grid.

    Each observation becomes one row in the output CSV.
        ...
        for station in stations:
            ...
            for r in profile_rows:
                ...
                rows.append({
                    ...
                    "pressure": r["Pres_dbar"],

                    "temperature": r["TEMP_QC_VAR"],

                    "salinity": r["SAL_QC_VAR"],
...
def _build_grid_training_df(grid_id: str) -> pd.DataFrame:

    rows = _grid_training_rows(grid_id)
...
def download_grid_csv(grid_id: str):
    """Downloadable training CSV for a single 5x5 degree grid cell
    (e.g. /grid-csv/Grid_137)."""
    if not _valid_grid_id(grid_id):
        raise HTTPException(status_code=400, detail=f"Invalid grid_id: '{grid_id}'")

    df = _build_grid_training_df(grid_id)
    if df.empty:
...
def download_all_grid_csvs():
    """All populated grids, bundled as one zip — for handing the full
    training set off to another team in one go."""
    buf = io.BytesIO()
    any_data = False
